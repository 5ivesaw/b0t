...
1. **Offline contracts and safety checks** compiles common and Forge-facing source for Java 8 against narrow verification stubs and runs 71 assertions covering contracts, safety, bounded state, terrain transforms, mid-range caching, conservative team classification, and the Phase 1 pipeline.
...
## Publishing `0.2.0-alpha.2`

Website method:

1. Open **Actions → Release → Run workflow**.
2. Enter `0.2.0-alpha.2` without the `v` prefix.
...
```powershell
git tag -a v0.2.0-alpha.2 -m "SawBotV1 Phase 1 internal eyes"
